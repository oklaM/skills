...
check_provider_uptime() {
    ...
    # 查找 Telegram provider 进程
    local provider_pid
    provider_pid=$(pgrep -f "openclaw.*provider.*telegram" 2>/dev/null | head -1)
    
    if [ -z "$provider_pid" ]; then
        log_warn "未找到 Telegram provider 进程"
        return 1
    fi
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        start_time=$(ps -p "$provider_pid" -o lstart= 2>/dev/null | xargs -I {} date -j -f "%c" "{}" +%s 2>/dev/null || echo "0")
    else
        start_time=$(ps -p "$provider_pid" -o lstart= 2>/dev/null | xargs -I {} date -d "{}" +%s 2>/dev/null || echo "0")
    fi
    
    if [ "$start_time" = "0" ] || [ -z "$start_time" ]; then
        log_warn "无法获取 provider 进程启动时间"
        return 0
    fi
    ...
    log_info "Telegram provider 运行时间: ${uptime_hours}小时"
    
    if [ "$uptime_hours" -gt "$threshold_hours" ]; then
        log_warn "Telegram provider 超过 ${threshold_hours} 小时无重启"
        return 1
    fi
